...
for i in xrange(0,steps):
	for j in xrange(0,layer_size):
		input_layer[j].input_snn_one_step(80+i_inj[j])
...
for i in xrange(0, steps):
	t = float(i) * tau
	weighted_conductance.append(util.linear_additive_conductance_at_t(spike_times, weights, t))

plt.figure()
plt.title("weighted conductance")
plt.plot(all_weighted_conductance, "r*")
plt.plot(weighted_conductance, "b-")
